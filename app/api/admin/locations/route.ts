...
export async function POST(request: NextRequest) {
  try {
    ...
    let locations: any[] = [];
    const stored = await redis.get('locations');
    if (stored) { locations = typeof stored === 'string' ? JSON.parse(stored) : stored; }
    else { locations = [...defaultLocations]; }
    const id = String(Date.now());
    const location = { id, neighborhood: newLoc.neighborhood, city: newLoc.city || '', state: newLoc.state || '', country: newLoc.country || 'USA', status: 'approved', order: locations.length + 1 };
