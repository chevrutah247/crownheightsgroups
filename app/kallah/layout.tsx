...
export default function KallahPage() {
  ...
  const getText = (en: string, he: string, ru: string) => {
    ...
    <div>
      ...
      <main className="main">
        ...
        <div style={{ background: 'linear-gradient(135deg, #fdf2f8 0%, #fce7f3 50%, #fbcfe8 100%)', borderRadius: '20px', padding: '2rem', marginBottom: '2rem', textAlign: 'center' }}>
          ...
          <h1 style={{ fontSize: '2rem', color: '#831843', marginBottom: '0.5rem' }}>
            {getText('Hachnasat Kallah', 'הכנסת כלה', 'Гахнасат Кала')}
          </h1>
          <p style={{ color: '#9d174d', fontSize: '1.1rem', marginBottom: '1rem' }}>
            {getText(
              'Helping brides marry with dignity and joy',
              'מצווה גדולה לעזור לכלות להתחתן בכבוד ובשמחה',
              'Заповедь помогать невестам выйти замуж достойно и радостно'
            )}
          </p>
          <p style={{ color: '#be185d', fontSize: '0.9rem', fontStyle: 'italic' }}>
            {getText(
              '📖 Talmud, Ketubot 67b — helping a bride is one of the highest forms of tzedakah',
              '📖 תלמוד, כתובות סז ע"ב - הכנסת כלה היא מצווה גדולה',
              '📖 Талмуд, Ктубот 67б — помощь невесте - высшая форма цдаки'
            )}
          </p>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '0.5rem', marginTop: '1.5rem' }}>
            {[
              ...
              <button key={l.code} onClick={() => setLang(l.code as any)} style={{ padding: '0.5rem 1rem', borderRadius: '20px', border: 'none', background: lang === l.code ? '#be185d' : 'white', color: lang === l.code ? 'white' : '#831843', cursor: 'pointer', fontWeight: lang === l.code ? 'bold' : 'normal' }}>
                {l.label}
              </button>
            ))}
          </div>
        ...
        <div style={{ marginBottom: '2rem' }}>
          <h2 style={{ fontSize: '1.3rem', marginBottom: '1rem', color: '#1e3a5f' }}>
            {getText('📱 WhatsApp Groups by Region', '📱 קבוצות וואטסאפ לפי אזור', '📱 WhatsApp группы по регионам')}
          </h2>
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <h2 style={{ fontSize: '1.3rem', marginBottom: '1rem', color: '#1e3a5f' }}>
            {getText('📋 Services Directory', '📋 מדריך שירותים', '📋 Каталог услуг')}
          </h2>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
            {categories.map(cat => (
              <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} style={{ padding: '0.5rem 1rem', borderRadius: '20px', border: 'none', background: selectedCategory === cat.id ? '#be185d' : '#fce7f3', color: selectedCategory === cat.id ? 'white' : '#831843', cursor: 'pointer', fontWeight: selectedCategory === cat.id ? 'bold' : 'normal' }}>
                {cat.icon} {getText(cat.name, cat.nameHe, cat.nameRu)}
              </button>
            ))}
          </div>
        </div>
          ...
          {filteredServices.map((service, index) => (
            <div key={index} style={{ background: 'white', borderRadius: '12px', padding: '1.25rem', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', border: '1px solid #fce7f3' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                <h3 style={{ margin: 0, fontSize: '1.1rem', color: '#831843' }}>
                  {getText(service.nameEn, service.name, service.nameRu)}
                </h3>
                <span style={{ padding: '2px 8px', background: '#fdf2f8', borderRadius: '4px', fontSize: '0.75rem', color: '#9d174d' }}>
                  📍 {lang === 'he' ? service.locationHe : service.location}
                </span>
              </div>
              
              <p style={{ color: '#666', fontSize: '0.9rem', margin: '0 0 1rem 0', minHeight: '40px' }}>
                {getText(service.descriptionEn, service.description, service.descriptionRu)}
              </p>
              
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <a href={`tel:${service.phone.replace(/-/g, '')}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.75rem', background: '#be185d', color: 'white', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold' }}>
                  📞 {service.phone}
                </a>
                {service.email && (
                  <a href={`mailto:${service.email}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.5rem', background: '#f3f4f6', color: '#374151', borderRadius: '8px', textDecoration: 'none', fontSize: '0.9rem' }}>
                    ✉️ {service.email}
                  </a>
                )}
                {service.address && (
        ...
        <div style={{ marginTop: '3rem', padding: '2rem', background: 'linear-gradient(135deg, #fdf2f8 0%, #fce7f3 100%)', borderRadius: '16px', textAlign: 'center' }}>
          <h3 style={{ color: '#831843', marginBottom: '1rem' }}>
            {getText('Know a bride who needs help?', 'מכירים כלה שצריכה עזרה?', 'Знаете невесту, которой нужна помощь?')}
          </h3>
          <p style={{ color: '#9d174d', marginBottom: '1.5rem' }}>
            {getText(
              'Share this page! It\'s a great mitzvah to help a bride.',
              'שתפו את הדף הזה! מצווה גדולה להפיץ',
              'Поделитесь этой страницей! Большая мицва помочь невесте.'
            )}
          </p>
          <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            <a href={`https://wa.me/?text=${encodeURIComponent('💒 Hachnasat Kallah - Help for Brides\nhttps://crownheightsgroups.com/kallah')}`} target="_blank" rel="noopener noreferrer" style={{ padding: '0.75rem 1.5rem', background: '#25D366', color: 'white', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold' }}>
              💬 {getText('Share on WhatsApp', 'שתפו בוואטסאפ', 'Поделиться в WhatsApp')}
            </a>
            <Link href="/groups" style={{ padding: '0.75rem 1.5rem', background: '#1e3a5f', color: 'white', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold' }}>
              👥 {getText('Browse All Groups', 'לכל הקבוצות', 'Все группы')}
            </Link>
          </div>
        </div>
      </main>
