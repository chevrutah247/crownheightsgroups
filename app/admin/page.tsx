          ...
          <div style={{background:'white',borderRadius:'12px',padding:'2rem',width:'90%',maxWidth:'600px',maxHeight:'90vh',overflow:'auto'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'1rem',marginTop:'1rem'}}>
              ...
              {modalType === 'group' && (<>
                ...
                <div><label style={labelStyle}>📷 Group Image</label><div style={{border:'2px dashed #ddd',borderRadius:'8px',padding:'1rem',textAlign:'center',background:'#fafafa'}}>{(imagePreview || editingItem.imageUrl) ? (<div><img src={imagePreview || editingItem.imageUrl} alt='Preview' style={{maxWidth:'100%',maxHeight:'150px',borderRadius:'8px'}} /><br/><button type='button' onClick={()=>{setImagePreview('');setEditingItem({...editingItem,imageUrl:''});}} style={{marginTop:'0.5rem',padding:'0.25rem 0.75rem',background:'#fee2e2',color:'#dc2626',border:'none',borderRadius:'4px',cursor:'pointer'}}>✕ Remove</button></div>) : (<label style={{cursor:'pointer',display:'block'}}><input type='file' accept='image/*' onChange={handleImageUpload} style={{display:'none'}} /><span style={{color:'#666'}}>📎 Click to upload</span><br/><span style={{fontSize:'0.8rem',color:'#999'}}>PNG, JPG up to 2MB</span></label>)}</div></div>
