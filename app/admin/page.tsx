...
interface Group { id: string; title: string; description: string; whatsappLink: string; categoryId: string; locationId: string; language: string; status: string; clicksCount: number; createdAt: string; isPinned?: boolean; }
interface User { id: string; email: string; name: string; role: 'user' | 'admin'; isVerified: boolean; createdAt: string; }
interface Service { id: string; name: string; phone: string; secondPhone?: string; categoryId: string; description?: string; languages?: string[]; locationId: string; status: string; isPinned?: boolean; }
...
export default function AdminPage() {
  ...
  const handleToggleAdmin = async (user: User) => {
    if (!confirm(`Change ${user.email} role?`)) return;
  ...
  // Group handlers
  const handleNewGroup = () => { setEditingGroup({ id: '', title: '', description: '', whatsappLink: '', categoryId: '1', locationId: '1', language: 'English', status: 'approved', clicksCount: 0, createdAt: '' }); setIsNewGroup(true); setShowGroupModal(true); };
  const handleEditGroup = (g: Group) => { setEditingGroup({ ...g }); setIsNewGroup(false); setShowGroupModal(true); };
  const handleSaveGroup = async () => {
    if (!editingGroup?.title) return alert('Title required');
    setSaving(true);
    await fetch('/api/admin/groups', { method: isNewGroup ? 'POST' : 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editingGroup) });
  ...
  // Service handlers
  const handleNewService = () => { setEditingService({ id: '', name: '', phone: '', categoryId: serviceCategories[0]?.id || '1', description: '', languages: ['English'], locationId: '1', status: 'approved' }); setIsNewService(true); setShowServiceModal(true); };
  ...
  const handleDeleteCategory = async (id: string) => {
    ...
    if (hasServices) return alert('Cannot delete - category has services. Delete or move services first.');
  ...
  const inputStyle = { padding: '0.75rem', borderRadius: '8px', border: '1px solid #ddd', width: '100%' };
          ...
          <>
            ...
            <div className="admin-card">
              <table className="admin-table">
                ...
                <tbody>
                  {serviceCategories.sort((a,b) => a.order - b.order).map(c => (
                    <tr key={c.id}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', borderRadius: '12px', padding: '2rem', width: '90%', maxWidth: '500px', maxHeight: '90vh', overflow: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1rem' }}>
              ...
              <input style={inputStyle} placeholder="WhatsApp Link" value={editingGroup.whatsappLink} onChange={e => setEditingGroup({ ...editingGroup, whatsappLink: e.target.value })} />
                ...
                <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem' }}>
                  {['English', 'Russian', 'Hebrew', 'Yiddish'].map(lang => (
                    <label key={lang}><input type="checkbox" checked={editingService.languages?.includes(lang)} onChange={e => {
                      ...
                      setEditingService({ ...editingService, languages: e.target.checked ? [...langs, lang] : langs.filter(l => l !== lang) });
                    }} /> {lang}</label>
                  ))}
                </div>
              </div>
      ...
      {showCategoryModal && editingCategory && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', borderRadius: '12px', padding: '2rem', width: '90%', maxWidth: '500px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1rem' }}>
              <div>
                <label style={{ fontWeight: 'bold' }}>Icon (emoji)</label>
                <input style={inputStyle} placeholder="🔧" value={editingCategory.icon} onChange={e => setEditingCategory({ ...editingCategory, icon: e.target.value })} />
              </div>
              <div>
                <label style={{ fontWeight: 'bold' }}>Name (English) *</label>
                <input style={inputStyle} placeholder="e.g. Plumber" value={editingCategory.name} onChange={e => setEditingCategory({ ...editingCategory, name: e.target.value })} />
              </div>
              <div>
                <label style={{ fontWeight: 'bold' }}>Name (Russian)</label>
                <input style={inputStyle} placeholder="e.g. Сантехник" value={editingCategory.nameRu || ''} onChange={e => setEditingCategory({ ...editingCategory, nameRu: e.target.value })} />
              </div>
            </div>
            <div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem' }}>
